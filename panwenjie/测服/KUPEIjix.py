...
class algoTestdemo(unittest.TestCase):
    ...
    def testwork(self):
        ...
        loginOrgResponsedata = requests.get('https://kupeitrial.t2.kupeiai.cn/__api/beatV3/intelligentstudy/'+str(chapterId)+'/chapter?source=2&roomId=8947',
                                             headers=headers)
        #获取taskExecutionId
        ...
        huoqkeid = requests.post('https://kupeitrial.t2.kupeiai.cn/__api/beatV3/student/task/22/'+str(taskExecutionId)+'/question'
        ...
        while num <3 :
            ...
            huoqutimuid = requests.put('https://kupeitrial.t2.kupeiai.cn/__api/beatV3/student/task/22/'+str(taskExecutionId)+'/answer', data=json.dumps(adsad),headers=headers)
            ...
            huoqkeid = requests.post(
                'https://kupeitrial.t2.kupeiai.cn/__api/beatV3/student/task/22/' + str(taskExecutionId) + '/question'
                , data=json.dumps(ijsji), headers=headers)
            print(huoqkeid.status_code)
            questionid = huoqkeid.json()['data']['questions'][0]['id']
            kpointId = huoqkeid.json()['data']['questions'][0]['kpointId']
        ...
        dasd = requests.get('https://kupeitrial.t2.kupeiai.cn/__api/beatV3/student/task/22/'+str(taskExecutionId)+'/report?isSummary=true',headers=headers)
        print(dasd.status_code)
        nihao2 = requests.get(
            'https://kupeitrial.t2.kupeiai.cn/__api/beatV3/intelligentstudy/ratelimitstatus?kpointId=' + str(kpointId),
            headers=headers)
