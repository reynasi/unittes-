...
class user_managementTest(unittest.TestCase):
    def setUp(self):
        global driver
    ...
    @BeautifulReport.add_test_img('test_user_management')
    def test_user_management(self):
        """user_management：机构管理后台登录"""
        ...
        try:
            ...
            is_element_exist(driver, '//*[@id="root"]/div/div[1]/div/img', project + '登录页')
            login(driver, mobil_xpath, code_xpath, btn_xpath, project)
            result = is_element_exist(driver, "//*[@id='root']/section/aside/div/div[1]/a/img", project + '登陆后元素存在判定')
            print('通过')
        except:
            traceback.print_exc()
        finally:
