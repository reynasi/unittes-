...
# @Time    : 2020/02/19
# @Author  : shuping zhao
# @File    : teacher4CustomizeTestingCase_Pro.py
import os
import sys
import traceback
import unittest
...
class teacher4CustomizeTestingCase_Pro(unittest.TestCase):
    def setUp(self):
        global browser
        global windows
    ...
    def test_b_customizeMathTesting(self):
        ...
        try:
            ...
            topicNum_ = shareUrl.split("chargeExam/", 2)
            # 获取教学测评的 executionId, topicId（使用招生测评的接口获取topicId，questionType）
            executionList = ApiCasePro.getTestingExecutionId(self, list3[0], topicNum_[1])
            while(elementflag):
                time.sleep(0.5)
                # 获取题目序号
                questionText = ""
...
#testsuit.addTest(teacher4CustomizeTestingCase_Pro("test_a_customizeEnglishTesting"))
testsuit.addTests(unittest.makeSuite(teacher4CustomizeTestingCase_Pro))
run = BeautifulReport(testsuit)
run.report(filename='TAD1.0正服_定制测评复制链接完成测评', description='TAD1.0正服_定制测评复制链接完成测评测试用例', report_dir=report_dir)