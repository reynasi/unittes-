...
class teacher4TestingCase_Pro(unittest.TestCase):
    def setUp(self):
        global browser
        global windows
    ...
    @BeautifulReport.add_test_img('test_a_englishTesting')
    def test_a_englishTesting(self):
        ''' 完成英语教学测评 '''
        try:
            time.sleep(2)
        ...
        print("-------------------> [1]英语教学测评结束 <---------------------\n")
    @BeautifulReport.add_test_img('test_b_mathTesting')
    def test_b_mathTesting(self):
        ''' 完成数学教学测评 '''
        try:
            time.sleep(2)
            topicNum = "jsJ7s80scFMCajQS1362991"
            topicId = "CMHJT006A0201"
...
#testsuit.addTest(teacher4TestingCase_Pro("test_b_mathTesting"))
testsuit.addTests(unittest.makeSuite(teacher4TestingCase_Pro))
run = BeautifulReport(testsuit)
run.report(filename='TAD1.0正服_教学测评复制链接完成测评', description='TAD1.0正服_教学测评复制链接完成测评测试用例', report_dir=report_dir)