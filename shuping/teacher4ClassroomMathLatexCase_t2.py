...
# @Time    : 2020/3/14
# @Author  : shuping zhao
# @File    : teacher4ClassroomMathLatexCase_t2.py
import sys
import time
import traceback
import unittest
...
class teacher4ClassroomMathLatexCase_t2(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            time.sleep(2)
            # 点击课程进入课堂
            self.findElementActionClick("//div[@class='cardWrapper']/div[1]/div[@class='lessonCardWrap']")
...
#   #获取元素后点击
    ...
    @BeautifulReport.add_test_img('test_a_assiginMathLatexCard')
    def test_a_assiginMathLatexCard(self):
        '''老师和学生都进入课堂学生完成数学公式闯关卡'''
        try:
            time.sleep(1)
            # 切换到教师端窗口
            browser.switch_to.window(windows[0])
            print("-------------------> 【Start】 老师开始发送闯关卡<----------------------\n")
            # 点击 Text_Lesson 1模块
            ...
            traceback.print_exc()
            print(e)
        self.assertEqual("单元1-数学公式测试题闯关卡", cardname, msg="学生端收到任务卡名称不对！")
        self.assertEqual("已完成", completeStatus, msg="完成数学公式闯关卡失败！")
        self.assertEqual(23, correctcount, "输入的答案没有完全判对")
        print("-------------------> 【End】 学生成功完成数学公式闯关卡<----------------------\n")
...
#testsuit.addTest(teacher4MathLatexCase_t2("test_a_assiginMobileExeciseCard"))
testsuit.addTests(unittest.makeSuite(teacher4MathLatexCase_t2))
run = BeautifulReport(testsuit)
# 服务器存储路径
run.report(filename='TAD1.0_t2_学生端上课完成数学公式闯关卡', description='TAD1.0_t2完成所有数学公式输入任务卡测试用例', report_dir=report_dir)
