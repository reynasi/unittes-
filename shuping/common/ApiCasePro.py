...
print("ApiCasePro__addon_dir: " + addon_dir)

class ApiCasePro(unittest.TestCase):
    # def setUp(self):
    ...
    def get_01(self, authorization, urlstr):
    ...
    def post_01(self, authorization, urlstr, questionId):
        #print("start post......")
    ...
    def getClassroomId(self, authorization, urlstr, orgId, searchName):
    ...
    def getTeacherxClassroomId(self, authorization, urlstr, orgId):
    ...
    def getTaskExecutionId(self, authorization, urlstr, classroomId):
    ...
    # 学生端获取questionId
    def get_studentquestion(self, authorization,urlstr, taskExecutionId):
    ...
    #  获取测评卡的topicId
    def getTopicId(self, authorization, urlstr, taskExecutionId):
    ...
    # 获取测评卡的goquestionId
    def getGoquestionId(self, authorization, urlstr, topicId):
    ...
    def getRequestionId(self, authorization, urlstr, taskExecutionId):
    ...
    def getDiyGoquestionId(self, authorization='Bearer 90de49c0-7c43-3c42-9547-1f397ec2c8ca', urlstr='shupingorg.learnta.cn', requestionId='409485'):
    ...
    def getActivityId(self, authorization, urlstr):  #
    ...
    def getTeacherxActivityId(self, authorization, urlstr):  #
        url = urlstr + "/__api/beatV3/recruit/activityList?pageNo=1&itemsPerPage=10&" \
    ...
    # 获取教学测评的executionId
    def getTestingExecutionId(self, authorization, topicNum):
        ...
        try:
            ...
            tasktype = json_response["taskType"]
            executionList = []
            executionList.append(executionId)
            executionList.append(tasktype)
        except:
            traceback.print_exc()
        return executionList

    # 获取招生测评的topicId  add data 20200409
    def getRecruitPreTopicIdNew(self, authorization, activityId, category, grade):  # category 代表科目 ， 每次获取的值都会变
        url = "https://learnta.cn/__api/beatV3/recruit/unCustom/topic/" + activityId
        ...
        try:
            ...
            json_response = json.loads(response.text)
            # print("Response_code：", response.status_code, "\n")
            pretopicId = json_response["data"]["categorys"][category]["grades"][grade]["topicId"]
        except:
            traceback.print_exc()
        return pretopicId

    # 获取招生测评的ExecutionId  update 20200409
    def getRecruitExecutionId(self, authorization, activityId, pretopicId): #category 代表科目 ， 每次获取的值都会变
        url = "https://www.learnta.cn/__api/beatV3/recruit/unCustom/goExam"

        headers = {"authorization": authorization,
                   "Content-Type": 'application/json'
                   }
        param = {"topicId": pretopicId,   # 1表示数学 ，0表示英语
        ...
        try:
            ...
            json_response = json.loads(response.text)
            # print("Response_code：", response.status_code, "\n")
            executionId = json_response["data"]["executionId"]
        except:
            traceback.print_exc()
        return executionId

    # 获取招生测评的topicId
    def getRecruitTopicId(self, authorization, executionId):
    ...
    # 获取招生测评的questiontype
    def getQuestionType(self, authorization, topicId):
    ...
    # 获取测评卡的questionId
    def getTestingquestionId(self, authorization, urlstr, goQuestionId):
    ...
    # 获取定制测评的questiontype
    def getCustomizeQuestionType(self, authorization, taskType, executionId):
        url = "https://learnta.cn/__api/beatV3/student/task/"+taskType+"/"+str(executionId)+"/question"

        headers = {"authorization": authorization,
                   "Content-Type": 'application/json'
                   }
        param = {"userAnswers":[], "isEvaluated": True,
                 "taskExecutionId": str(executionId)
                 }
        try:
            response = requests.post(url=url, headers=headers, json=param)
            json_response = json.loads(response.text)
            # print("Response_code：", response.status_code, "\n")
            if json_response["data"]["questions"] == None:
                questiontype=2    # 以及结束到结果页
            else:
                questiontype = json_response["data"]["questions"][0]["type"]
        except:
            traceback.print_exc()
        return questiontype
...
# testsuite.addTest(ApiTestCasePro("getTopicId"))
