...
class teacher4MobileMathLatexCase_t2(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            time.sleep(0.5)
            # 输入自动化测试移动课堂 搜索课程
            browser.find_element_by_xpath("//input[@type='text']").send_keys("自动化测试数学课程数学公式")
            # 点击搜索按钮
...
#   #获取元素后点击
    ...
    @BeautifulReport.add_test_img('test_a_assiginMobileMathExeciseCard_pro')
    def test_a_assiginMobileMathExeciseCard_pro(self):
        '''移动课堂完成数学公式练习卡'''
        try:
            # 输入闯关卡搜索内容，点击搜索按钮
            ...
            # 重命名任务名称
            browser.find_element_by_xpath("//input[@placeholder='请编辑任务名称']").send_keys("自动化测试移动课堂数学公式练习卡")
            time.sleep(0.3)
            # 点击确定按钮
            ...
            traceback.print_exc()
            print("移动课堂完成数学公式练习卡失败！")
        self.assertEqual(23, correctcount, msg="完成练习卡失败！")
...
# 服务器存储路径
run.report(filename='TAD1.0正服_移动课堂完成所有数学公式练习卡', description='TAD1.0正服_移动课堂完成所有数学公式练习卡测试用例', report_dir=report_dir)
